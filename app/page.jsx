...
export default function Home() {
  ...
  const topRatedPosters = topRatedMoviePosterPaths.map((path, index) => (
    <ImagePoster path={path} key={index} num={index + 1} classes="h-auto w-[145px] sm:w-[145px] md:w-[170px] lg:w-[200px] xl:w-[225px] rounded-2xl" mainClasses="h-auto w-[175px] sm:w-[175px] md:w-[200px] lg:w-[230px] xl:w-[255px]" />
  ));

  const popularPosters = popularMoviePosterPaths.map((path, index) => (
    <ImagePoster path={path} key={index} classes="h-auto w-[200px] sm:w-[90px] md:w-[100px] rounded-2xl" />
  ));
  ...
  return (
    <div className='w-full h-full pt-12 px-6'>
      <h1 className="text-white font-bold text-3xl">What do you want to watch?</h1>
